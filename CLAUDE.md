...
- **yabai stale lock file** — Rapid restarts can leave `/tmp/yabai_<user>.lock` behind, causing yabai to abort mid-config. Fix: `rm -f /tmp/yabai_$(whoami).lock && yabai --start-service`. Check `/tmp/yabai_$(whoami).err.log` for "could not acquire lock-file" errors.
- **tmux copy mode** — Use `prefix + [` to enter, `q` to exit; mouse selection auto-copies

## yabai Rule Tips

- Use `title!="<regex>"` (with `!` before `=`) to negate title matches in a single rule — avoids two competing rules that can cause feedback loops
- Never use two rules with the same `app` but opposing `manage` values (e.g., `manage=off` then `manage=on`) — this can cause windows to rapidly flash/move as the rules fight each other combined with focus/rebalance signals
- Rule syntax reference: `app[!]=`, `title[!]=`, `role[!]=`, `subrole[!]=` all support negation
